...
class Login extends CI_Model {
	...
	public function addAuthToken($token, $start, $end)
	{
		return $this->db->insert('auth', array(
							'auth_token' => $token,
							'auth_time_creat' => $start,
							'auth_time_elapsed' => $end)
							);
	}
	
	public function getAuthToken($token){
		$array = null;
		$query = $this->db->query('SELECT * FROM auth WHERE auth_token="'.$token.'"');
